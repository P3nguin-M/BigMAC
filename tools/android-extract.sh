...
at_unzip()
{
	echo "Reached at_unzip with args $1, $2"
	local filename="$1"
	local dir="$2"
	...
	if [ "$format" == "zip" ] || [ "$format" == "ZIP" ] || [ "$format" == "Zip" ]; then
		...
		if [ -z "$dir" ]; then
			...
			unrar x "$filename"
			if [ "$vendor" == "samsung" ]; then
				echo "Untaring samsung md5 file"
				tar xvf `basename "$filename" ".rar"`".md5"
			fi
			rm "$filename"
			cd "$bakfromrar"
...
handle_ext4()
{
	...
	cp "$img" $DIR_TMP/"$ext"
	# NOTE: needs sudo or root permission
	echo "Mounting image: $(basename $img)"
...
handle_simg()
{
	...
	# NOTE: needs sudo or root permission
	echo "Mounting: $1 to $DIR_TMP/$ext"
	sudo mount -o ro -t ext4 $DIR_TMP/"$ext" $mnt_name
	# Find the boot.oat for RE odex
	BOOT_OAT=""
	BOOT_OAT_64=""
...
#-------------------------------------------------------------------------------
		...
		if [[ "$justname" == *".lz4" ]] ; then
			...
			echo "new decompressed file name is: $newfilename"
...
echo "Output will be available in: $MY_DIR/$SUB_DIR_"

mkdir $SUB_DIR
mv "$IMAGE" $SUB_DIR
cd $SUB_DIR
echo "check files in path then press any key"
read
# Try to unzip
...
#-------------------------------------------------------------------------------
	...
	if [ "$DECSUFFIX" == ".zip" ]; then
	...
	elif [[ "$DECSUFFIX" == *".tar" ]]; then
		echo ".tar file extraction"
		tar -xvf $IMAGE --directory=$SUB_SUB_TMP
		cd $SUB_SUB_TMP
		for i in $(ls|grep lz4);
			do 
				echo "Decompressing lz4 file: $i";
				lz4 -d $i;
				rm $i;
		done 
		cd ..

	elif [[ "$DECSUFFIX" == *".md5" ]]; then
		echo ".md5 file extraction"
		tar -xvf $IMAGE --directory=$SUB_SUB_TMP
		...
		echo "was not any of those for samsung!!"
	fi
	cd $SUB_SUB_TMP
